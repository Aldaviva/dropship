...
exports.getLatestBuild = function(project){
	...
	var rawJobBuildDataPromise = _getJSON(project.ci.url+"/lastBuild/api/json");
	var rawComponentsBuildDataPromises = _.map(project.components, function(component){
		return _getJSON(project.ci.url+'/'+component.package+'/lastBuild/api/json');
	});
...
function getBuildState(raw){
	return (raw.building) ? "in progress" : raw.result.toLowerCase();
}
